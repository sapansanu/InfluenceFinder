...
def find_L1_follower_list(screen_name):
    ...
    while True:
        try:
            ...
            print(e)
            print('Follower list: RATE LIMIT - waiting 15 minute...')
            time.sleep(60*15)
            follower = next(followers)
    ...
    for i in range(1):
        try:
            ...
            if "Not authorized" in str(e):
                ...
                print(e)
                print('Follower detail: RATE LIMIT - waiting 15 minute...')
                time.sleep(60*15)
                return
        for tweet in timeline:
                if (("python" in tweet.text or "Python" in tweet.text) and not tweet.retweeted):
                    ...
                    tweet_count += 1
    print("Tweet count for L1 follower "+follower_screen_name+" : "+str(tweet_count))
    if tweet_count > 0:
        with open(finder.path+user_screen_name+".json","r") as file:
            data = json.load(file)
            data['follower_list'] += [follower_screen_name]
            finder.write_json(user_screen_name, data, finder.path)
        print("Searching follower details..")
        ...
        finder.write_json(follower_screen_name, data, finder.followers_L1_path)
        #Finding level 2 followers
        f2.find_L2_follower_list(follower_screen_name)
